...
// Core Node.js modules for HTTP requests, file operations, and path handling
...
// loadEnv() => loads environment variables from .env file manually
function loadEnv() {
 // Path to .env file in the same directory as this script
 const envPath = path.resolve(__dirname, '.env');
 // Exit early if .env file doesn't exist
 if (!fs.existsSync(envPath)) return;

 // Read and parse .env file content line by line
 ...
 for (const line of lines) {
  ...
  // Set environment variable if both key and value exist
...
// Flag to prevent multiple custom model loads
let customModelsLoaded = false;

// loadCustomModels() => dynamically loads additional models from _models.js file
async function loadCustomModels() {
 // Prevent loading custom models multiple times
 if (customModelsLoaded) return;
 
 try {
  // Try to import custom models configuration
  ...
  if (fs.existsSync(customModelsPath)) {
   ...
   // Merge custom models into existing providers
   if (module._MODELS) {
...
// Define which models support vision/image processing capabilities
const VISION_MODELS = {
...
// Define which models support reasoning/thinking capabilities
...
// detectImageFormat() => analyzes buffer to determine image MIME type
function detectImageFormat(buffer) {
 // Return default JPEG if buffer is invalid
...
// Convert various image formats to data URL format
...
// Check if provider/model combination supports vision capabilities
...
// Check if provider/model combination supports thinking capabilities
function supportsThinking(provider, model) {
 const models = THINKING_MODELS[provider];
 return models && models.includes(model);
}

// Configure thinking/reasoning parameters for different AI providers
function getThinkingConfig(provider, model, thinkValue) {
 // Return null if thinking is disabled or not specified
 ...
 // Grok provider uses reasoning_effort parameter
 if (provider === 'grok') {
 ...
 // Convert thinking level to token budget for other providers
 let budget = 0;
 ...
 // Return provider-specific thinking configuration
 if (budget > 0) {
  if (provider === 'gemini') {
...
// Normalize message content to consistent format for all providers
...
// Configuration object containing all AI provider settings and capabilities
const AI_PROVIDERS = {
 openai: {
  apiKey: process.env.OPENAI_API_KEY || "",
  baseUrl: 'https://api.openai.com/v1/chat/completions',
  // Model definitions with pricing information
  models: {
  ...
  formatPayload: (messages, model, maxTokens, options = {}) => {
   ...
   // Process vision requests with image format conversion
   if (isVision) {
   ...
   // Handle reasoning models with special parameters
 ...
 anthropic: {
  ...
  // Anthropic model configurations with pricing
  models: {
  ...
  formatPayload: (messages, model, maxTokens, options = {}) => {
   ...
   for (const msg of messages) {
    // Extract system message separately for Anthropic format
    if (msg.role === 'system') {
    ...
    if (msg.role === 'user' || msg.role === 'assistant') {
     // Handle vision messages with image processing
   ...
   // Handle edge case where only system message exists
   ...
   // Add system message to payload if present
 ...
 mistral: {
  ...
  // Mistral model configurations with pricing
  models: {
  ...
  formatPayload: (messages, model, maxTokens, options = {}) => {
   ...
   let processedMessages = messages;
   // Convert image formats for Mistral API
   if (isVision) {
    processedMessages = messages.map(msg => {
     if (Array.isArray(msg.content)) {
      return {
 ...
 grok: {
  ...
  // Grok model configurations with pricing
  models: {
...
'grok-3m': { name: 'grok-3-mini-latest', cost: { in: 0.3, out: 0.5 } }
 ...
 gemini: {
  ...
  // Gemini model configurations with pricing
  models: {
  ...
  formatPayload: (messages, model, maxTokens, options = {}) => {
   ...
   for (const msg of messages) {
    ...
    // Handle vision content with image processing
    if (isVision && Array.isArray(msg.content)) {
   ...
   // Handle edge case with only system message
   if (convertedMessages.length === 0 && systemMessage) {
...
// Normalize usage statistics from different providers to standard format
function normalizeUsage(provider, usage, messages, responseText) {
 ...
 // Handle Gemini-specific usage format
 ...
 // Handle Anthropic-specific usage format
...
// Send HTTPS POST request to AI provider endpoints
...
export async function callAI(...args) {
 ...
 // Parse different argument patterns for flexible API calls
 if (args.length === 4) {
 ...
 // Load custom models if not already loaded
 ...
 // Check if vision request is supported by the model
